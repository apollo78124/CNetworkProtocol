...
static void parse_arguments(int argc, char *argv[], struct options *opts)
{
    ...
    {
        if ((argc - 1 - optind) > 19) {
            fatal_message(__FILE__, __func__ , __LINE__, "\"Too many file arguments", 5);
        }
        int tempFEIndex = 0;
        for(char **tempv = argv+ optind; *tempv != argv[argc]; tempv++) {
            opts->file_expressions[tempFEIndex] = *tempv;
            tempFEIndex++;
        }

        readFile(opts->file_expressions);
    }
